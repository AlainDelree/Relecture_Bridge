...
Date:   Thu Aug 27 18:26:06 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #495 : issues_inbox devient methode principale, formulaire web en backup
    
    - Contenu du watcher issues_inbox/ (ex-§20) deplace en §3 (methode
      principale), sous-sections renumerotees 3.1-3.11
    - Contenu du formulaire web new_issue.py (ex-§3) deplace en §20
      (methode de backup), avec paragraphe explicite sur son role
    - §1 : nouveau bloc d'introduction annoncant la hierarchie des
      qu'au sommaire du document
    - Renvois croises mis a jour dans tout le document (§6, §10, §11,
      §12, §14) et pied de page glisse (issue #495 en tete)
    - Aucune perte d'information, contenu integralement conserve des
      deux cotes
    - Complete un premier essai qui avait timeout (400s) apres avoir
      pose une partie des renvois croises sans deplacer le contenu

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/BRIDGE_AGENT_DOC.md b/BRIDGE_AGENT_DOC.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 0efdd83..74130dd 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/BRIDGE_AGENT_DOC.md
# ── Version APRÈS ce commit.
+++ b/BRIDGE_AGENT_DOC.md
# ── Zone modifiée : ligne 92 (182 ligne(s)) dans l'ancienne version → ligne 92 (264 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -92,182 +92,264 @@ et son propre journal de log (`logs/watcher-<nom>.log`).
 
 ---
 
-## 3. Créer une issue — la méthode normale
+## 3. Créer une issue — méthode principale : watcher `issues_inbox` (issue #483)
...
+nettoie — sans repasser par le formulaire web. **C'est la méthode
+principale recommandée pour créer une issue depuis Claude Chat** ; le
+formulaire web reste documenté comme méthode de backup (§20).
...
# ── Zone modifiée : ligne 2301 (247 ligne(s)) dans l'ancienne version → ligne 2383 (184 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -2301,247 +2383,184 @@ issues de la même combinaison s'il le juge utile.
...
-  sans ouvrir le fichier. Laissés en place pour correction manuelle — le
-  watcher ne les retraite jamais automatiquement.
+## 20. Créer une issue — méthode de backup : formulaire web `new_issue.py`
 
-### 20.3 Format attendu du fichier
+> Méthode de **backup** — la méthode principale est le dépôt d'un
+> fichier dans `issues_inbox/` (§3). Ce formulaire reste utile en cas
+> d'indisponibilité du watcher spool, pour une création manuelle par
+> Alain directement dans le navigateur, ou pour l'aperçu de la commande
+> avant envoi (fonctionnalité propre au formulaire, sans équivalent
+> côté spool).
...
+*Dernière mise à jour : 27 août 2026 — restructuration §3/§20 (issue #495) :
+le watcher `issues_inbox/` (auparavant §20) devient la méthode **principale**
+de création d'issue depuis Claude Chat et prend la place du §3 (avec ses
+sous-sections renumérotées 3.1 à 3.11) ; le formulaire web `new_issue.py`
+(auparavant §3) devient la méthode de **backup**, documentée au nouveau §20
+— toujours intégralement fonctionnelle (aperçu de la commande, création
+manuelle par Alain, repli si le watcher spool est indisponible). Contenu
+intégralement conservé des deux côtés, seule la présentation relative
+change : nouveau bloc d'introduction au §1, renvois croisés mis à jour dans
+tout le document (§6, §10, §11, §12, §14).
+Précédemment — 26 août 2026 — §3.4 « Anti-doublon » (issue #491) :
...
+Précédemment — 25 août 2026 — §3.10 « Pilotage du watcher depuis
...
# ── Zone modifiée : ligne 2550 (40 ligne(s)) dans l'ancienne version → ligne 2569 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -2550,40 +2569,6 @@ lancement CLI manuel non suivi. `app/issues_inbox.py` gère le fichier PID
...
diff --git a/CHANGELOG-495.md b/CHANGELOG-495.md
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# (index — ignorable)
index 0000000..ab34a57
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/CHANGELOG-495.md
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (37 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,37 @@
+## 27 août 2026 — issue #495
+
+Restructuration de `BRIDGE_AGENT_DOC.md` : `issues_inbox/` devient la
+méthode **principale** de création d'issue depuis Claude Chat, le
+formulaire web `new_issue.py` devient la méthode de **backup** —
+l'usage réel (dépôt direct d'un fichier plutôt que copier-coller manuel)
+ne correspondait plus au titre « §3, la méthode normale » porté jusqu'ici
+par le formulaire web, ni à l'ordre des sections (watcher `issues_inbox`
+documenté tout en fin de doc, au §20).
+- Contenu du watcher `issues_inbox/` (ex-§20, sous-sections 20.1 à 20.11)
+  déplacé en §3, sous-sections renumérotées 3.1 à 3.11 ; nouveau titre
+  « 3. Créer une issue — méthode principale : watcher `issues_inbox` ».
+  Paragraphe « Objectif » (§3.1) complété d'une phrase explicite sur le
+  statut de méthode principale recommandée.
+- Contenu du formulaire web (ex-§3) déplacé en §20, nouveau titre
+  « 20. Créer une issue — méthode de backup : formulaire web
+  `new_issue.py` », avec un court paragraphe d'introduction précisant le
+  statut de backup et les cas d'usage propres au formulaire (aperçu de la
+  commande avant envoi, création manuelle par Alain, repli si le watcher
+  spool est indisponible).
+- Nouveau bloc au §1 « Vue d'ensemble » annonçant dès l'introduction la
+  hiérarchie principale (§3) / backup (§20).
+- Tous les renvois croisés du document mis à jour en conséquence (§6
+  « Champs spéciaux », §10 « Structure du dépôt », §11 « Conventions de
+  code », §12 « Règles d'usage » — workflow et couverture universelle des
+  consignes injectées, §14 « Délégation Chef → Ouvrier »).
+- Pied de page de `BRIDGE_AGENT_DOC.md` glissé (issue #495 en tête, #491
+  et #485 conservées comme les deux entrées les plus récentes parmi les
+  issues modifiant cette doc — renumérotées §3.4/§3.10 pour rester
+  cohérentes avec le nouveau plan ; #483 et #446 sorties, elles restent
+  disponibles ici).
+
+Aucune perte d'information : les deux méthodes restent intégralement
+documentées, seule leur présentation relative (numérotation, titres,
+ordre d'apparition) change. Tâche relancée après un premier essai en
+timeout (400s), qui avait déjà posé une partie des renvois croisés sans
+effectuer le déplacement de contenu lui-même — travail complété ici.
