...
Date:   Sat Aug 22 08:27:00 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Supprimer le code mort dupliqué des modes pédagogique et labo (issue #201)
    
    Suppression de nicsoft/modes/pedagogique/__main__ (pedagogique).py et
    nicsoft/modes/labo/__main__labo.py, deux copies orphelines non importées
    ailleurs (grep exhaustif sur .py/.sh/.bat/.ps1/.github confirmant l'absence
    de toute référence), remplacées de longue date par pedagogique.py et
    labo.py qui en sont des évolutions strictement plus complètes (i18n,
    RodentEngine, pipeline de précalcul, kill_switch, BackMenuExit).

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/CHANGELOG-201.md b/CHANGELOG-201.md
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 0000000..821a13e
...
diff --git a/nicsoft/modes/labo/__main__labo.py b/nicsoft/modes/labo/__main__labo.py
# ── Ce fichier est supprimé par ce commit.
deleted file mode 100755
# (index — ignorable)
index fe721db..0000000
# (avant — fichier suivant)
--- a/nicsoft/modes/labo/__main__labo.py
# (après — fichier suivant)
+++ /dev/null
# ── Zone modifiée : ligne 1 (409 ligne(s)) dans l'ancienne version → ligne 0 (0 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1,409 +0,0 @@
-"""
...
-        except Exception: pass
# (diff du fichier suivant)
diff --git a/nicsoft/modes/pedagogique/__main__ (pedagogique).py b/nicsoft/modes/pedagogique/__main__ (pedagogique).py
# ── Ce fichier est supprimé par ce commit.
deleted file mode 100755
# (index — ignorable)
index 54eb5ff..0000000
# (avant — fichier suivant)
--- a/nicsoft/modes/pedagogique/__main__ (pedagogique).py	
# (après — fichier suivant)
+++ /dev/null
# ── Zone modifiée : ligne 1 (1691 ligne(s)) dans l'ancienne version → ligne 0 (0 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1,1691 +0,0 @@
-"""
