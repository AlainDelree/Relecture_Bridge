...
Date:   Fri Aug 7 17:55:36 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Corrige logs silencieux et timeout inadapté au téléchargement ZIP (issue #31)
    
    - version_check.py, mise_a_jour.py, notifications.py : remplace
      _LOGGER.debug par _LOGGER.warning dans tous les except réseau
      (invisible auparavant car le logger tourne à INFO en production)
    - actualise.py : ajoute un log INFO au démarrage de
      tache_verification_arriere_plan(), pour distinguer un thread qui ne
      démarre pas d'un thread qui tourne sans rien trouver
    - version_check.py : sépare TIMEOUT_RESEAU_SECONDES (3s) en
      TIMEOUT_VERSION_SECONDES (5s, JSON léger) et
      TIMEOUT_DOWNLOAD_SECONDES (60s, zip 30-80 Mo via GitHub Releases) ;
      met à jour les imports dans mise_a_jour.py et notifications.py
...
index 53b0c15..9d50dce 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/actualise.py
# ── Version APRÈS ce commit.
+++ b/actualise.py
# ── Zone modifiée : ligne 307 (6 ligne(s)) dans l'ancienne version → ligne 307 (8 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 39 (18 ligne(s)) dans l'ancienne version → ligne 39 (18 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 30 (7 ligne(s)) dans l'ancienne version → ligne 30 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index 81cc68c..bba8f5d 100644
# (avant — fichier suivant)
--- a/version_check.py
# (après — fichier suivant)
+++ b/version_check.py
# ── Zone modifiée : ligne 9 (12 ligne(s)) dans l'ancienne version → ligne 9 (18 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 30 (7 ligne(s)) dans l'ancienne version → ligne 36 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -30,7 +36,7 @@ def verifier_version(depot_github: str, build_installe: int) -> dict[str, Any] |
...
# ── Zone modifiée : ligne 40 (17 ligne(s)) dans l'ancienne version → ligne 46 (17 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -40,17 +46,17 @@ def verifier_version(depot_github: str, build_installe: int) -> dict[str, Any] |
