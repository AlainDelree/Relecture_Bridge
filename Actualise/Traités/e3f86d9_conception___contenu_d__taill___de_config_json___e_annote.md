...
Date:   Sat Jul 25 22:02:14 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    conception : contenu détaillé de config.json + emplacement zone d'attente (issue #7, chef #207)

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/CONCEPTION.md b/CONCEPTION.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 9cc192f..f11e691 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/CONCEPTION.md
# ── Version APRÈS ce commit.
+++ b/CONCEPTION.md
# ── Zone modifiée : ligne 112 (6 ligne(s)) dans l'ancienne version → ligne 112 (60 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -112,6 +112,60 @@ raisons :
...
# ── Zone modifiée : ligne 246 (15 ligne(s)) dans l'ancienne version → ligne 300 (10 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -246,15 +300,10 @@ sans nécessiter de compteur ni d'état persistant à gérer.
