...
Date:   Thu Aug 6 17:34:07 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #381 : panneau flottant — monitoring CCL, actions, pastilles projets
    
    - Monitoring : bouton "Lancer les éteints" (renommé) + nouveau "Relancer
      tous les CCL" (tous, actifs ou éteints), résumé "en cours/en file" par
      projet, horodatage du dernier rafraîchissement.
    - Actions : mode de l'issue affiché, bouton "Interrompre et relancer"
      (adapté CCL/CCW), bouton "Fermer l'issue" sur needs-human.
    - Filtres projet : pastille de notification (issues ouvertes non
      terminées) + bouton "Cocher tout".

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/CHANGELOG.md b/CHANGELOG.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index fce5c2a..0179367 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/CHANGELOG.md
# ── Version APRÈS ce commit.
+++ b/CHANGELOG.md
# ── Zone modifiée : ligne 9 (6 ligne(s)) dans l'ancienne version → ligne 9 (47 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index cbb0323..aac0c97 100644
# (avant — fichier suivant)
--- a/static/css/style.css
# (après — fichier suivant)
+++ b/static/css/style.css
# ── Zone modifiée : ligne 114 (10 ligne(s)) dans l'ancienne version → ligne 114 (17 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -114,10 +114,17 @@ button.danger:hover{background:#f8d7da}
...
+.filtre-projet{position:relative;display:inline-flex;align-items:center;gap:6px;font-size:12px;
...
+/* Pastille de notification (issue #381) : nombre d'issues ouvertes ni done ni
+   needs-human de ce projet, visible même quand le bouton est inactif (filtre
+   masqué) — c'est justement le cas d'usage principal (savoir qu'un projet
+   masqué a des issues en attente sans avoir à le réafficher). */
+.filtre-projet .pastille-notif{position:absolute;top:-6px;right:-6px;min-width:15px;height:15px;
+  padding:0 3px;border-radius:8px;background:#c0392b;color:#fff;font-size:10px;font-weight:700;
+  line-height:15px;text-align:center;box-shadow:0 0 0 1px #fff}
...
# ── Zone modifiée : ligne 153 (6 ligne(s)) dans l'ancienne version → ligne 160 (11 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -153,6 +160,11 @@ button.danger:hover{background:#f8d7da}
   border-radius:14px;background:#fff;color:#555;cursor:pointer;user-select:none;
   transition:opacity .12s}
 .btn-rafraichir:hover{opacity:.6}
+/* Bouton « Cocher tout » (issue #381), même gabarit que les boutons de filtre. */
+.btn-cocher-tout{font-size:12px;padding:5px 11px;border:1px solid #ccc;
+  border-radius:14px;background:#fff;color:#555;cursor:pointer;user-select:none;
+  transition:opacity .12s}
+.btn-cocher-tout:hover{opacity:.6}
...
# ── Zone modifiée : ligne 305 (6 ligne(s)) dans l'ancienne version → ligne 317 (15 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -305,6 +317,15 @@ button.danger:hover{background:#f8d7da}
...
+/* Boutons de relance groupée CCL côte à côte (issue #381) : « Lancer les
+   éteints » (seulement si au moins un est éteint) et « Relancer tous les CCL »
+   (toujours, actifs ou non). */
+.pl-boutons-ccl{display:flex;gap:6px;flex-wrap:wrap;margin:6px 0 2px}
+.pl-boutons-ccl .pl-btn-vm{margin:0;flex:1}
+/* Mode de l'issue sélectionnée (issue #381), en haut de la zone actions. */
+.pl-mode-issue{font-size:13px;font-weight:600;color:#1a1a18;margin-bottom:10px}
...
# (index — ignorable)
index 2e72feb..ec29904 100644
# (avant — fichier suivant)
--- a/static/js/app.js
# (après — fichier suivant)
+++ b/static/js/app.js
# ── Zone modifiée : ligne 948 (6 ligne(s)) dans l'ancienne version → ligne 948 (17 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 957 (6 ligne(s)) dans l'ancienne version → ligne 968 (57 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 1231 (6 ligne(s)) dans l'ancienne version → ligne 1293 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 1648 (6 ligne(s)) dans l'ancienne version → ligne 1711 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 1826 (15 ligne(s)) dans l'ancienne version → ligne 1890 (42 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+// d'en-tête.
 async function rendrePanneauLateralMonitoring() {
   ...
   if (!zone) return;
# ── Zone modifiée : ligne 1885 (22 ligne(s)) dans l'ancienne version → ligne 1976 (32 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 1919 (6 ligne(s)) dans l'ancienne version → ligne 2020 (13 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 1929 (6 ligne(s)) dans l'ancienne version → ligne 2037 (18 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+
 function rendrePanneauLateralActions() {
   ...
   if (!zone) return;
# ── Zone modifiée : ligne 1943 (17 ligne(s)) dans l'ancienne version → ligne 2063 (35 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1943,17 +2063,35 @@ function rendrePanneauLateralActions() {
...
# ── Zone modifiée : ligne 2016 (7 ligne(s)) dans l'ancienne version → ligne 2154 (32 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 3205 (11 ligne(s)) dans l'ancienne version → ligne 3368 (14 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 3222 (7 ligne(s)) dans l'ancienne version → ligne 3388 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+             + ' Continuer ?')) return false;
   ...
   let resultat;
   try {
# ── Zone modifiée : ligne 3234 (11 ligne(s)) dans l'ancienne version → ligne 3400 (11 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 3254 (6 ligne(s)) dans l'ancienne version → ligne 3420 (25 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -3254,6 +3420,25 @@ async function interrompreIssue(nom, numero) {
