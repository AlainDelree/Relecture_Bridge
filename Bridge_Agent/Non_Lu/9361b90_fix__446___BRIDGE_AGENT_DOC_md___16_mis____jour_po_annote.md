...
Date:   Tue Aug 18 11:01:47 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #446 : BRIDGE_AGENT_DOC.md §16 mis à jour pour CCW sur PC fixe physique

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/BRIDGE_AGENT_DOC.md b/BRIDGE_AGENT_DOC.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 574a0f9..ba7b43c 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/BRIDGE_AGENT_DOC.md
# ── Version APRÈS ce commit.
+++ b/BRIDGE_AGENT_DOC.md
# ── Zone modifiée : ligne 43 (7 ligne(s)) dans l'ancienne version → ligne 43 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -43,7 +43,7 @@ Claude Chat → crée une issue → GitHub → watcher.py détecte → CCL exéc
 > lui-même** :
...
# ── Zone modifiée : ligne 1212 (23 ligne(s)) dans l'ancienne version → ligne 1212 (35 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1212,23 +1212,35 @@ finale.
...
# ── Zone modifiée : ligne 1267 (13 ligne(s)) dans l'ancienne version → ligne 1279 (13 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1267,13 +1279,13 @@ le dépôt à titre historique uniquement ; ne pas les utiliser.
...
# ── Zone modifiée : ligne 1281 (15 ligne(s)) dans l'ancienne version → ligne 1293 (23 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1281,15 +1293,23 @@ le dépôt à titre historique uniquement ; ne pas les utiliser.
...
# ── Zone modifiée : ligne 1327 (18 ligne(s)) dans l'ancienne version → ligne 1347 (24 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1327,18 +1347,24 @@ LIBELLE_AGENT = agent Windows
...
# ── Zone modifiée : ligne 1349 (7 ligne(s)) dans l'ancienne version → ligne 1375 (8 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1349,7 +1375,8 @@ python3 provisioning/windows/lancer_provisioning.py             # copie + exécu
...
# ── Zone modifiée : ligne 1357 (7 ligne(s)) dans l'ancienne version → ligne 1384 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1357,7 +1384,7 @@ authentifier Claude (`ANTHROPIC_API_KEY` en variable d'environnement, ou
...
# ── Zone modifiée : ligne 1442 (6 ligne(s)) dans l'ancienne version → ligne 1469 (15 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1442,6 +1469,15 @@ Fine-grained tokens) :
...
# ── Zone modifiée : ligne 1531 (6 ligne(s)) dans l'ancienne version → ligne 1567 (16 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1531,6 +1567,16 @@ la VM (`creer_vm_ccw.py --recreate` + ré-attacher un ISO frais +
...
# ── Zone modifiée : ligne 1588 (42 ligne(s)) dans l'ancienne version → ligne 1634 (58 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1588,42 +1634,58 @@ nécessaire — l'issue est directe.
...
# ── Zone modifiée : ligne 1631 (7 ligne(s)) dans l'ancienne version → ligne 1693 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1631,7 +1693,7 @@ projet buildé (dont Scrabble) — voir `BUILD_WINDOWS_CCW.md`.
...
# ── Zone modifiée : ligne 1655 (7 ligne(s)) dans l'ancienne version → ligne 1717 (15 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1655,7 +1717,15 @@ fichier existe, même après redémarrage.
    Remove-Item C:\CCW\Bridge_Agent\logs\verrous\<fichier>.lock
    ```
 
-**Bouton « Interrompre » (issue #323, implémenté).** Le bouton ⛔ « Interrompre
...
# ── Zone modifiée : ligne 2185 (14 ligne(s)) dans l'ancienne version → ligne 2255 (32 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -2185,14 +2255,32 @@ issues de la même combinaison s'il le juge utile.
 
 ---
 
-*Dernière mise à jour : 13 août 2026 — §3 « Convention de présentation côté
-Claude Chat » (issue #443) : la règle du bloc de code unique (issue #153),
-jusqu'ici formulée pour le mode lot seulement, est étendue explicitement au
-cas mono-issue — une issue unique est elle aussi présentée dans un bloc de
-code, afin qu'Alain puisse utiliser le bouton copier du bloc. §11
-« Conventions de code » : le bullet « Issues » précise désormais que le
-corps est toujours présenté dans un bloc de code, qu'il s'agisse d'une issue
-seule ou d'un lot.
...
# ── Zone modifiée : ligne 2200 (13 ligne(s)) dans l'ancienne version → ligne 2288 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -2200,13 +2288,6 @@ champ `RESEAU` documenté (issue #435, `oui`/`non`, lu par
