...
Date:   Tue Sep 15 11:17:40 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #546 : cause reelle du blocage Add-Type identifiee (classifieur CLI code en dur, non pilotable par --allowedTools), retrait de l'entree ; allowedTools passe en argument unique joint par virgules pour git pull --ff-only

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/watcher.py b/watcher.py
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 740fc34..607f4f2 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/watcher.py
# ── Version APRÈS ce commit.
+++ b/watcher.py
# ── Zone modifiée : ligne 175 (16 ligne(s)) dans l'ancienne version → ligne 175 (16 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -175,16 +175,16 @@ MODE_LECTURE        = "lecture"         # défaut : diagnostic, aucune écriture
...
# ── Zone modifiée : ligne 200 (22 ligne(s)) dans l'ancienne version → ligne 200 (50 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -200,22 +200,50 @@ MODE_ECRITURE       = "ecriture"        # écriture libre dans REP_TRAVAIL (labe
...
# ── Zone modifiée : ligne 2422 (11 ligne(s)) dans l'ancienne version → ligne 2450 (14 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -2422,11 +2450,14 @@ Si la tâche échoue, remplace ✅ par ❌ et explique la cause en une ligne.
