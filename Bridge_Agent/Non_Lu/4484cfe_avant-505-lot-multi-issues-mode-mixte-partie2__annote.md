...
Date:   Sun Aug 30 16:36:07 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    avant-505-lot-multi-issues-mode-mixte-partie2

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/static/js/app.js b/static/js/app.js
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index cba2202..ad41e25 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/static/js/app.js
# ── Version APRÈS ce commit.
+++ b/static/js/app.js
# ── Zone modifiée : ligne 4680 (8 ligne(s)) dans l'ancienne version → ligne 4680 (12 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -4680,8 +4680,12 @@ function reconnaitreModeTexte(brut) {
...
# ── Zone modifiée : ligne 4804 (21 ligne(s)) dans l'ancienne version → ligne 4808 (38 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -4804,21 +4808,38 @@ function projetEffectifBloc(bloc, projetForm) {
...
+function modeEffectifBloc(bloc, modeForm) {
...
+      if (m && !modes.includes(m)) modes.push(m);
     ...
     let suffixe = '';
     if (projets.length === 1) {
# ── Zone modifiée : ligne 4826 (6 ligne(s)) dans l'ancienne version → ligne 4847 (11 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -4826,6 +4847,11 @@ function mettreAJourBoutonLot() {
...
# ── Zone modifiée : ligne 4863 (7 ligne(s)) dans l'ancienne version → ligne 4889 (8 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -4863,7 +4889,8 @@ function afficherResumeLot(resultats, projetForm) {
...
# ── Zone modifiée : ligne 4920 (8 ligne(s)) dans l'ancienne version → ligne 4947 (12 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -4920,8 +4947,12 @@ async function envoyerLot() {
...
# ── Zone modifiée : ligne 4935 (13 ligne(s)) dans l'ancienne version → ligne 4966 (14 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -4935,13 +4966,14 @@ async function envoyerLot() {
