...
Date:   Tue Aug 25 12:39:32 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #485 : contrôle watcher issues_inbox (spool) depuis panneau Infrastructure
    
    Complète le commit de sauvegarde (déjà porteur de l'implémentation
    backend/frontend : app/issues_inbox.py, scripts/watcher_issues_inbox.py,
    app/__init__.py, static/js/app.js, templates/index.html) en ajoutant la
    documentation manquante §20.10 de BRIDGE_AGENT_DOC.md et le changelog.
    Vérifié fonctionnellement : démarrage avec durée, relance (tue l'ancien
    process et repart avec la nouvelle durée), arrêt propre (PID + échéance
    nettoyés).

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/BRIDGE_AGENT_DOC.md b/BRIDGE_AGENT_DOC.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 7ed5aad..ec0145b 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/BRIDGE_AGENT_DOC.md
# ── Version APRÈS ce commit.
+++ b/BRIDGE_AGENT_DOC.md
# ── Zone modifiée : ligne 2394 (9 ligne(s)) dans l'ancienne version → ligne 2394 (77 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -2394,9 +2394,77 @@ aucun appel `gh`, aucune dépendance à un watcher en cours d'exécution) :
