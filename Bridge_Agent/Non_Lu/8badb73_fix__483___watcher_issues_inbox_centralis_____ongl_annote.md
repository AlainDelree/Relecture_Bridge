...
Date:   Tue Aug 25 11:36:47 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #483 : watcher issues_inbox centralisé + onglet Résultats inbox
    
    Nouveau flux de création d'issues sans passage par le formulaire web :
    scripts/watcher_issues_inbox.py scrute issues_inbox/, parse et valide
    l'en-tête, crée l'issue via gh issue create, journalise dans
    logs/issues_inbox.log (rotation à 50 lignes), déplace les fichiers
    invalides vers issues_inbox/rejected/. app/issues_inbox.py expose l'état
    (route /issues-inbox/etat) au nouvel onglet « Résultats inbox » de
    new_issue.py, alarme pilotée uniquement par l'état de rejected/.
    Documentation ajoutée en BRIDGE_AGENT_DOC.md §20 + CHANGELOG.md.

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/.gitignore b/.gitignore
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index b8749fe..d361111 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/.gitignore
# ── Version APRÈS ce commit.
+++ b/.gitignore
# ── Zone modifiée : ligne 11 (3 ligne(s)) dans l'ancienne version → ligne 11 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -11,3 +11,6 @@ provisioning/windows/autounattend.local.xml
...
# (index — ignorable)
index f84a878..7ed5aad 100644
# (avant — fichier suivant)
--- a/BRIDGE_AGENT_DOC.md
# (après — fichier suivant)
+++ b/BRIDGE_AGENT_DOC.md
# ── Zone modifiée : ligne 443 (8 ligne(s)) dans l'ancienne version → ligne 443 (11 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 2279 (7 ligne(s)) dans l'ancienne version → ligne 2282 (129 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 2304 (14 ligne(s)) dans l'ancienne version → ligne 2429 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 130 (4 ligne(s)) dans l'ancienne version → ligne 131 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index 0000000..e58a257
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/app/issues_inbox.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (69 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,69 @@
...
new file mode 100644
# (index — ignorable)
index 0000000..0a5aee6
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/scripts/watcher_issues_inbox.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (443 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,443 @@
+#!/usr/bin/env python3
+"""
...
# (index — ignorable)
index 4c6b0f1..f16d104 100644
# (avant — fichier suivant)
--- a/static/css/style.css
# (après — fichier suivant)
+++ b/static/css/style.css
# ── Zone modifiée : ligne 454 (3 ligne(s)) dans l'ancienne version → ligne 454 (17 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -454,3 +454,17 @@ button.danger-plein:hover{background:#8f2626}
...
+/* Onglet « Résultats inbox » (issue #483) : alarme pilotée UNIQUEMENT par
+   l'état (vide/non-vide) de issues_inbox/rejected/ — badge clignotant sur
+   l'onglet lui-même (visible même hors de cette vue) + bandeau dans le panneau. */
+@keyframes clignoter-alarme{0%,100%{opacity:1}50%{opacity:.25}}
+.badge-alarme-inbox{display:inline-block;margin-left:4px;font-size:13px;
+  animation:clignoter-alarme 1.1s ease-in-out infinite}
+.alarme-inbox{padding:12px 14px;border-radius:6px;font-size:13px;
+  margin-bottom:14px;background:#f8d7da;color:#721c24;
+  border:1px solid #f5c6cb;font-weight:600;
+  animation:clignoter-alarme 1.6s ease-in-out infinite}
+.inbox-historique{max-height:320px;overflow-y:auto;font-family:monospace;
+  font-size:12px;white-space:pre-wrap;background:#f8f8f5;
+  border:1px solid #e0dfda;border-radius:6px;padding:10px 12px;color:#444}
+#corps-inbox-rejetes td{padding:6px 0;font-size:13px;border-bottom:1px solid #f0efe9}
# (diff du fichier suivant)
diff --git a/static/js/app.js b/static/js/app.js
# (index — ignorable)
index e878c3f..54e0e3a 100644
# (avant — fichier suivant)
--- a/static/js/app.js
# (après — fichier suivant)
+++ b/static/js/app.js
# ── Zone modifiée : ligne 69 (7 ligne(s)) dans l'ancienne version → ligne 69 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+  const noms = ['creation', 'resultats', 'inbox', 'journal', 'config', 'watchers', 'ccw'];
   ...
   noms.forEach(n =>
# ── Zone modifiée : ligne 92 (6 ligne(s)) dans l'ancienne version → ligne 92 (10 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 4924 (6 ligne(s)) dans l'ancienne version → ligne 4928 (55 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -4924,6 +4928,55 @@ async function lancerWatcher() {
 })();
...
# (index — ignorable)
index b86edf8..d2fe6e9 100644
# (avant — fichier suivant)
--- a/templates/index.html
# (après — fichier suivant)
+++ b/templates/index.html
# ── Zone modifiée : ligne 73 (6 ligne(s)) dans l'ancienne version → ligne 73 (9 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -73,6 +73,9 @@
   <div class="onglets">
...
+    <div class="onglet" onclick="basculerOnglet('inbox')">Résultats inbox
+      <span id="badge-alarme-inbox" class="badge-alarme-inbox" style="display:none" title="Des fichiers issues_inbox/rejected/ attendent une correction manuelle">🚨</span>
+    </div>
...
# ── Zone modifiée : ligne 286 (6 ligne(s)) dans l'ancienne version → ligne 289 (38 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -286,6 +289,38 @@
 
   </div>
 
+  <!-- ─── Onglet « Résultats inbox » (issue #483) : état du watcher
+       issues_inbox/ — alarme pilotée UNIQUEMENT par la présence de fichiers
+       dans issues_inbox/rejected/ (pas de parsing de log), rafraîchie
+       périodiquement par rafraichirInbox() (static/js/app.js) via
+       /issues-inbox/etat, indépendamment de l'onglet actif (le badge sur
+       l'onglet lui-même doit rester à jour même hors de cette vue). -->
+  <div id="panneau-inbox" class="panneau">
+    <div id="alarme-inbox" class="alarme-inbox" style="display:none">
+      🚨 Des issues déposées dans <code>issues_inbox/</code> ont été rejetées —
+      correction manuelle requise dans <code>issues_inbox/rejected/</code>.
+    </div>
+    <div id="inbox-sans-alarme" class="message succes" style="display:block">✅ Aucun fichier en attente de correction dans <code>issues_inbox/rejected/</code>.</div>
+
+    <div class="titre-section">Fichiers rejetés</div>
+    <table style="width:100%;border-collapse:collapse" id="tableau-inbox-rejetes">
+      <thead>
+        <tr style="border-bottom:1px solid #eee">
+          <th style="text-align:left;font-size:12px;color:#666;font-weight:500;padding:8px 0">Fichier</th>
+          <th style="text-align:left;font-size:12px;color:#666;font-weight:500;padding:8px 0;width:200px">Déposé le</th>
+        </tr>
+      </thead>
+      <tbody id="corps-inbox-rejetes"></tbody>
+    </table>
+
+    <div class="titre-section" style="margin-top:18px">Historique (logs/issues_inbox.log)</div>
+    <div id="inbox-historique" class="inbox-historique"></div>
+
+    <div style="display:flex;justify-content:flex-end;margin-top:12px">
+      <button onclick="rafraichirInbox()">Rafraîchir</button>
+    </div>
+  </div>
+
