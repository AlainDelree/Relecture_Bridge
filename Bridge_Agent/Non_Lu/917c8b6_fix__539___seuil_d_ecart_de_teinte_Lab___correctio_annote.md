...
Date:   Sun Sep 13 09:30:17 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #539 : seuil d'ecart de teinte Lab + correction ciblee des 3 (+1) paires trop proches
    
    Le seuil de garde CIE76 >= 15 de #535 ne suffisait pas : les 3 paires
    signalees (alchess/rummikub, ecole/chesscoach, actualise/gestionmail) ont
    une distance CIE76 largement au-dessus du seuil (56 et 52 mesures) mais un
    ecart d'angle de teinte Lab quasi nul (1,6 et 0,4 degres) - meme teinte,
    clarte/chroma differents, ce que l'oeil lit comme une nuance et pas deux
    couleurs sur une petite pastille.
    
    - Ajout de SEUIL_ECART_TEINTE_MIN (15 deg) verifie par generer_palette() en
      plus de SEUIL_DISTANCE_MIN (remonte 15->20, defense en profondeur)
    - generer_palette() accepte couleurs_a_eviter, utilise pour que la palette
      proposee aux futurs projets respecte aussi les couleurs figees en dur
      (trou qui avait laisse passer gestionmail)
    - Correction ciblee de 4 couleurs dans COULEURS_PROJETS_EXISTANTS (alchess,
      ecole, actualise, + bloc_score/bridge_agent decouverte via le nouveau
      seuil) ; les 7 autres inchangees. static/js/app.js resynchronise.
...
index 0000000..87479bf
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- /dev/null
# ── Version APRÈS ce commit.
+++ b/CHANGELOG-539.md
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (77 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,77 @@
...
# (index — ignorable)
index 3bd591e..747655e 100755
# (avant — fichier suivant)
--- a/nouveau_projet.py
# (après — fichier suivant)
+++ b/nouveau_projet.py
# ── Zone modifiée : ligne 49 (11 ligne(s)) dans l'ancienne version → ligne 49 (11 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -49,11 +49,11 @@ LABELS = [
...
# ── Zone modifiée : ligne 65 (6 ligne(s)) dans l'ancienne version → ligne 65 (26 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -65,6 +65,26 @@ LABELS = [
...
# ── Zone modifiée : ligne 79 (16 ligne(s)) dans l'ancienne version → ligne 99 (27 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -79,16 +99,27 @@ LABELS = [
...
# ── Zone modifiée : ligne 156 (65 ligne(s)) dans l'ancienne version → ligne 187 (117 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 229 (25 ligne(s)) dans l'ancienne version → ligne 312 (74 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -229,25 +312,74 @@ PALETTE_COULEURS = generer_palette(NB_COULEURS_PALETTE)
...
# (index — ignorable)
index d389e7e..4a3f58e 100644
# (avant — fichier suivant)
--- a/static/js/app.js
# (après — fichier suivant)
+++ b/static/js/app.js
# ── Zone modifiée : ligne 31 (19 ligne(s)) dans l'ancienne version → ligne 31 (23 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
