...
Date:   Thu Aug 27 19:15:24 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #497 : BRIDGE_AGENT_DOC.md — restructure §3/§20 sur l'état actuel de master
    
    Watcher issues_inbox/ (ex-§20, sous-sections 20.1-20.12 incluant le cycle
    de vie lié à new_issue.py de #496) devient la méthode principale, §3
    (sous-sections 3.1-3.12). Formulaire web new_issue.py (ex-§3) devient la
    méthode de backup, §20, avec un paragraphe de tête sur son statut et ses
    cas d'usage propres. Tous les renvois croisés du document (§1, §6, §10,
    §11, §12, §14, footer) mis à jour vers la nouvelle numérotation. Refait
    sur master actuel la restructuration tentée par #495 (worktree abandonné,
    fusion trop fragile avec #496) ; entrée de changelog couvrant #495+#496.
...
index a40648c..507e684 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/BRIDGE_AGENT_DOC.md
# ── Version APRÈS ce commit.
+++ b/BRIDGE_AGENT_DOC.md
# ── Zone modifiée : ligne 81 (181 ligne(s)) dans l'ancienne version → ligne 81 (325 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -81,181 +81,325 @@ et son propre journal de log (`logs/watcher-<nom>.log`).
...
# ── Zone modifiée : ligne 312 (7 ligne(s)) dans l'ancienne version → ligne 456 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -312,7 +456,7 @@ Le watcher lit ces champs dans le tableau markdown de l'en-tête :
...
# ── Zone modifiée : ligne 337 (7 ligne(s)) dans l'ancienne version → ligne 481 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -337,7 +481,7 @@ Format dans le corps :
...
# ── Zone modifiée : ligne 445 (11 ligne(s)) dans l'ancienne version → ligne 589 (11 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -445,11 +589,11 @@ méthode par défaut recommandée.
...
# ── Zone modifiée : ligne 639 (7 ligne(s)) dans l'ancienne version → ligne 783 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -639,7 +783,7 @@ curl -sL "https://raw.githubusercontent.com/AlainDelree/Bridge_Agent/master/cons
...
# ── Zone modifiée : ligne 1156 (7 ligne(s)) dans l'ancienne version → ligne 1300 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1156,7 +1300,7 @@ create`, puis surveille sa fermeture avant de livrer sa réponse.
...
# ── Zone modifiée : ligne 2284 (362 ligne(s)) dans l'ancienne version → ligne 2428 (195 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -2284,362 +2428,195 @@ issues de la même combinaison s'il le juge utile.
