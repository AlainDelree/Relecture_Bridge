...
Date:   Sun Aug 2 18:19:27 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #332 : avertir des vérifications working tree quand l'issue interrompue écrivait

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/CHANGELOG.md b/CHANGELOG.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index b2f5594..9eedffd 100644
...
# (index — ignorable)
index 6297c81..110d9a9 100644
# (avant — fichier suivant)
--- a/static/js/app.js
# (après — fichier suivant)
+++ b/static/js/app.js
# ── Zone modifiée : ligne 2732 (6 ligne(s)) dans l'ancienne version → ligne 2732 (36 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+
 async function interrompreIssue(nom, numero) {
   ...
   if (!depot) {
# ── Zone modifiée : ligne 2740 (12 ligne(s)) dans l'ancienne version → ligne 2770 (16 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+             + ' Continuer ?')) return;
   ...
   let resultat;
   try {
# ── Zone modifiée : ligne 2763 (7 ligne(s)) dans l'ancienne version → ligne 2797 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 2786 (7 ligne(s)) dans l'ancienne version → ligne 2820 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 2816 (6 ligne(s)) dans l'ancienne version → ligne 2850 (10 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -2816,6 +2850,10 @@ function ouvrirModalInterrompre(resultat) {
