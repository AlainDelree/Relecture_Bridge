...
Date:   Sun Aug 2 22:29:23 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #334 : fetch unique de l'issue au décompte TIMEOUT + 15s
    
    Onglet Résultats : au lieu de laisser le badge figé sur « ⌛ 0s — budget
    épuisé », un unique fetch de /issue/<projet>/<numero> est programmé 15s
    après le dépassement du décompte TIMEOUT (marge laissée au watcher pour
    poster son diagnostic et fermer l'issue). Issue fermée → ligne mise à
    jour normalement (comme un ↻ manuel sur cette seule ligne). Issue encore
    ouverte (cas marginal) → badge « rafraîchir ↻ », aucun autre fetch
    automatique. Zéro polling : un seul appel réseau par issue, une seule
    fois, garanti par un Set garde-fou même si le composant est re-rendu.

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/CHANGELOG.md b/CHANGELOG.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index def9792..d764995 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/CHANGELOG.md
# ── Version APRÈS ce commit.
+++ b/CHANGELOG.md
# ── Zone modifiée : ligne 9 (6 ligne(s)) dans l'ancienne version → ligne 9 (36 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index 110d9a9..90b7970 100644
# (avant — fichier suivant)
--- a/static/js/app.js
# (après — fichier suivant)
+++ b/static/js/app.js
# ── Zone modifiée : ligne 1163 (6 ligne(s)) dans l'ancienne version → ligne 1163 (32 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+
 function rendreListeIssues(reset) {
   ...
   zone.innerHTML = '';
# ── Zone modifiée : ligne 1173 (33 ligne(s)) dans l'ancienne version → ligne 1199 (8 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 1358 (6 ligne(s)) dans l'ancienne version → ligne 1359 (18 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 1490 (7 ligne(s)) dans l'ancienne version → ligne 1503 (9 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+function formaterBadgeTempsRestant(badge, t, projet, numero) {
   ...
   badge.style.display = '';
# ── Zone modifiée : ligne 1531 (22 ligne(s)) dans l'ancienne version → ligne 1546 (34 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 1560 (10 ligne(s)) dans l'ancienne version → ligne 1587 (78 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1560,10 +1587,78 @@ function majBadgesTempsRestant() {
