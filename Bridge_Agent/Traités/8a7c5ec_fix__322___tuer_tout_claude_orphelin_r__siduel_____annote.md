...
Date:   Sun Aug 2 15:17:49 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #322 : tuer tout claude orphelin résiduel à la reprise d'un verrou périmé
    
    Ferme le dernier trou du cycle de vie verrou/claude : si le watcher meurt
    brutalement (kill -9, coupure, plantage non capturé) pendant qu'un claude
    tourne, aucun finally ne s'exécute — le claude devient orphelin et le
    verrou reste posé. Au démarrage suivant, la reprise du verrou périmé tue
    désormais cet orphelin (garde-fou anti-reboot à trois conditions : verrou
    périmé + process vivant + c'est bien un claude), avant de relancer un
    nouveau claude dans le même REP_TRAVAIL.
...
index 0109ed8..0260429 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/CHANGELOG.md
# ── Version APRÈS ce commit.
+++ b/CHANGELOG.md
# ── Zone modifiée : ligne 9 (6 ligne(s)) dans l'ancienne version → ligne 9 (60 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index 0000000..9c99361
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/tests/test_orphelin_verrou_perime_322.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (358 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,358 @@
+#!/usr/bin/env python3
...
# (index — ignorable)
index ae2cc6a..0755c1b 100644
# (avant — fichier suivant)
--- a/watcher.py
# (après — fichier suivant)
+++ b/watcher.py
# ── Zone modifiée : ligne 1854 (7 ligne(s)) dans l'ancienne version → ligne 1854 (8 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 1873 (6 ligne(s)) dans l'ancienne version → ligne 1874 (13 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1873,6 +1874,13 @@ def lancer_claude(numero: int, titre: str, body: str, dry_run: bool,
...
+
     ...
     """
     if timeout is None:
# ── Zone modifiée : ligne 2038 (6 ligne(s)) dans l'ancienne version → ligne 2046 (12 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 2160 (6 ligne(s)) dans l'ancienne version → ligne 2174 (104 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 2201 (6 ligne(s)) dans l'ancienne version → ligne 2313 (15 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 2479 (7 ligne(s)) dans l'ancienne version → ligne 2600 (8 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
