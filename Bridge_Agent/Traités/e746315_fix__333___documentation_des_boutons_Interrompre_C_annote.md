...
Date:   Sun Aug 2 22:14:08 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix #333 : documentation des boutons Interrompre CCL et CCW (implémentés)

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/BRIDGE_AGENT_DOC.md b/BRIDGE_AGENT_DOC.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index ead755f..d6a5777 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/BRIDGE_AGENT_DOC.md
# ── Version APRÈS ce commit.
+++ b/BRIDGE_AGENT_DOC.md
# ── Zone modifiée : ligne 806 (6 ligne(s)) dans l'ancienne version → ligne 806 (54 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -806,6 +806,54 @@ dans le `.conf` du projet.
...
# ── Zone modifiée : ligne 1512 (9 ligne(s)) dans l'ancienne version → ligne 1560 (22 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1512,9 +1560,22 @@ fichier existe, même après redémarrage.
...
# ── Zone modifiée : ligne 1976 (6 ligne(s)) dans l'ancienne version → ligne 2037 (47 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1976,6 +2037,47 @@ issues de la même combinaison s'il le juge utile.
...
# (index — ignorable)
index 9eedffd..def9792 100644
# (avant — fichier suivant)
--- a/CHANGELOG.md
# (après — fichier suivant)
+++ b/CHANGELOG.md
# ── Zone modifiée : ligne 9 (6 ligne(s)) dans l'ancienne version → ligne 9 (32 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
