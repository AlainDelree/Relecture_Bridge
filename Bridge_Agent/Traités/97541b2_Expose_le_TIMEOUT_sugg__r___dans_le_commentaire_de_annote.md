...
Date:   Sat Jul 25 02:50:35 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Expose le TIMEOUT_suggéré dans le commentaire de clôture GitHub (issue #222)
    
    - lire_timeout_suggere() : lecture seule (sans écrire ni verrouiller)
      du TIMEOUT_suggéré courant pour une combinaison (projet, TYPE, mode),
      à partir de l'état déjà persisté par maj_calibration_timeout (#221).
    - formater_bloc_calibration() : bloc lisible durée réelle + TIMEOUT_suggéré.
    - editer_dernier_commentaire() : gh issue comment --edit-last, pour
      enrichir a posteriori le commentaire de résultat déjà posté (succès),
      une fois la durée réelle et donc TIMEOUT_suggéré connus.
    - Succès : le commentaire de résultat est édité après fermeture pour y
      ajouter le bloc calibration (calcul frais, maj_calibration_timeout).
    - Échec définitif : le bloc est ajouté au message avant de le poster,
      en LECTURE SEULE (lire_timeout_suggere) — chaque tentative expirée a
      déjà mis à jour l'EWMA dans la boucle de retry ; un nouvel appel à
      maj_calibration_timeout ici compterait deux fois la même observation.

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/watcher.py b/watcher.py
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 851f672..3d1d947 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/watcher.py
# ── Version APRÈS ce commit.
+++ b/watcher.py
# ── Zone modifiée : ligne 1024 (6 ligne(s)) dans l'ancienne version → ligne 1024 (58 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1024,6 +1024,58 @@ def maj_calibration_timeout(*, projet: str, type_issue: str, mode: str,
     return suggere
...
# ── Zone modifiée : ligne 1221 (6 ligne(s)) dans l'ancienne version → ligne 1273 (33 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1221,6 +1273,33 @@ def commenter_issue(numero: int, message: str) -> bool:
...
# ── Zone modifiée : ligne 1768 (10 ligne(s)) dans l'ancienne version → ligne 1847 (10 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1768,10 +1847,10 @@ def traiter_issue(issue: dict, dry_run: bool):
...
# ── Zone modifiée : ligne 1813 (7 ligne(s)) dans l'ancienne version → ligne 1892 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1813,7 +1892,7 @@ def traiter_issue(issue: dict, dry_run: bool):
...
+                suggere = maj_calibration_timeout(
                     ...
                     mode=mode_close,
# ── Zone modifiée : ligne 1823 (6 ligne(s)) dans l'ancienne version → ligne 1902 (16 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1823,6 +1902,16 @@ def traiter_issue(issue: dict, dry_run: bool):
                     body=body,
...
# ── Zone modifiée : ligne 1903 (6 ligne(s)) dans l'ancienne version → ligne 1992 (17 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1903,6 +1992,17 @@ def traiter_issue(issue: dict, dry_run: bool):
