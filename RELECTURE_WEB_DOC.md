...
| **Merger** | Une ou plusieurs branches sélectionnées. | `git merge <branche>` (avec bascule temporaire si la branche cible n'a pas de worktree dédié) | **Confirmation légère** (issue #71 ; fenêtre maison depuis l'issue #73, plus `confirm()` natif) — modifie la branche cible mais reste local et annulable tant que le merge n'est pas finalisé (section 10). Refusé si la branche est déjà la branche cible, ou si le diagnostic « doublons uniquement » (section 4) indique qu'elle n'apporterait aucun contenu nouveau. **Garde-fou avant tout merge (issue #74) : refusé par message flash explicite si le dossier de la branche cible contient des modifications non committées (fichiers suivis modifiés, ou fichiers non suivis non ignorés — `git status --porcelain`)** — un tel état peut signaler qu'une tâche CCL y travaille encore, repliée sur ce même dossier faute de worktree dédié disponible (voir issue #73, où l'intégration automatique du CHANGELOG avait alors committé ce travail en cours par-dessus) ; aucune tentative de fusion n'est même lancée dans ce cas. Un conflit (une fois le merge lancé) laisse volontairement le dépôt en état de fusion non résolue (pas de rollback automatique), pour ne pas perdre l'information. Même timeout étendu (120s) que Push (issue #39) ; en cas de dépassement, l'état réel est revérifié automatiquement (branche source devenue ancêtre de la cible ? fusion arrêtée sur des conflits ? ni l'un ni l'autre ?) et le message flash final le reflète explicitement, plutôt qu'un renvoi systématique vers une vérification manuelle (issue #66). Projet à plusieurs cibles configurées (section 7, issue #52) : un sélecteur « Cible du merge » apparaît dans le panneau d'actions — aucune cible n'est devinée, refus par message flash si aucune n'est choisie explicitement. Même indicateur « Fusion en cours... Ns » que Push après confirmation (issue #49). Si le merge introduit un ou plusieurs `CHANGELOG-<N>.md` à la racine du dépôt, une intégration automatique dans `CHANGELOG.md` est tentée juste après (issue #51) ; succès ou échec est rapporté par message flash **et** journalisé dans `relecture_web/changelog_fusion.log` (date/heure, projet, commande, résultat, erreur éventuelle) — consultable après coup même si le flash est passé inaperçu (issue #62). **Le script utilisé est toujours celui de relecture_bridge (`scripts/fusionner_changelog.py`, ce dépôt), jamais une copie du projet fusionné (issue #87)** — résolu à partir de l'emplacement de `relecture_web/git_info.py` lui-même, puis lancé avec `--repo <dépôt fusionné>` ; auparavant, seuls quelques projets équipés d'une copie locale du script bénéficiaient de cette intégration, les autres échouaient avec « script introuvable ». Ce script reste robuste à un `CHANGELOG.md` cible qui ne suit pas la convention « Convention d'ajout : ... » de relecture_bridge (issue #252) : en-tête absent -> insertion après le premier titre de niveau 1 trouvé, ou en tête de fichier si aucun titre ; `CHANGELOG.md` absent -> créé avec pour seul contenu les entrées fusionnées ; contenu existant jamais perdu, quelle que soit la forme du fichier cible. **Portée strictement limitée à `CHANGELOG.md` et aux `CHANGELOG-<N>.md` consommés (issue #74)** : le commit d'intégration (`git add -- CHANGELOG.md CHANGELOG-<N>.md... && git commit -- <mêmes chemins>`) ne touche jamais que ces fichiers précis — jamais un `git commit -a`/`git add -A`, qui aurait embarqué toute autre modification présente dans l'arbre (cause du commit 0c477c0 pendant l'issue #73, qui avait ainsi committé le travail inachevé d'une tâche CCL repliée sur ce même dossier). |
