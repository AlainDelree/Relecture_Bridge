...
- **`branche` accepte plusieurs branches séparées par une virgule**
  ...
  cibles s'affiche donc sans erreur. **Reste non traité (issue #51) : le
  bouton Merger**, dont la commande affichée (`commande_merge`) pour une
  branche d'un tel projet ne choisit pas automatiquement entre les
  cibles — vérifier manuellement la commande avant de l'exécuter dans ce
  cas précis, le reste de la page n'étant pas concerné.
