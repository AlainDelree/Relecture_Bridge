...
Date:   Sun Aug 23 18:05:23 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Extrait onclick/style inline de #screen-ouvertures vers data-action + CSS (issue #221)
    
    Remplace les 4 attributs onclick= du bloc #screen-ouvertures par le
    mécanisme data-action (issue #220) : bouton retour → nouveau case
    "back_menu" dans le listener délégué, carte Exercices → menu_card_launch
    existant (data-mode="exercices"), carte Opening Explorer → send_mode
    existant (data-mode="opening_explorer").
    
    Cas particulier de la checkbox "Mode virtuel" (onclick="event.stopPropagation()") :
    remplacé par une vérification en tête du listener délégué central
    (e.target.closest(".menu-card-chk-label") → sortie immédiate avant
    dispatch de l'action de carte), approche suggérée par l'issue — évite
    tout stopPropagation() par label individuel.
    
    Remplace les 8 style= inline par des classes dédiées dans main.css
    (#screen-ouvertures, .ouvertures-header/.ouvertures-h2/.ouvertures-cards/
    .ouvertures-card/.ouvertures-chk-label), en réutilisant .btn-continuer
    déjà existant pour le bouton retour (même valeurs exactes).
    
    Met à jour go_exercices() dans test_smoke_e2e.py : sélecteur
    .outil-card-clickable[data-mode="exercices"] au lieu de
    onclick*="'exercices'".
    
    Suite e2e complète (42 passed, 1 skipped) et suite pytest complète
    (78 passed, 1 skipped) au vert après modification. Vérification manuelle
    Playwright ad-hoc : clic sur la checkbox toggle son état sans lancer la
    carte, clic sur la carte lance bien Exercices.
