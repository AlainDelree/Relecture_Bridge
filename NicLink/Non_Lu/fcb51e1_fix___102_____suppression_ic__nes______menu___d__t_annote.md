...
Date:   Mon Aug 10 21:38:06 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    fix: #102 — suppression icônes 🖥 menu + détection déconnexion plateau en session
    
    Tâche 1 : retrait de l'emoji 🖥 sur menu.btn.analyse et menu.btn.retranscrire
    (fr/en/de.json).
    
    Tâche 2 : le plateau débranché en cours de session n'était jamais signalé
    au frontend — _fen_reader_loop avalait silencieusement les erreurs USB.
    - hid_backend.py : get_fen() catche OSError/IOError sans lever, réinitialise
      _dev/_current_fen, et bascule le nouveau flag module _connected à False ;
      ajout de is_connected().
    - driver.py : _fen_reader_loop vérifie is_connected() après chaque get_fen(),
      compte les échecs consécutifs, et après 5 échecs appelle
      self._board_lost_cb() puis arrête le thread.
    - board_adapter.py : câble _board_lost_cb sur send_event("board_error", ...)
      pour toute instance physique créée via create_board().
    - server.py : reconnect_board (bouton "Connecter") est désormais intercepté
      directement dans on_action, qu'on soit au menu ou en session — réutilise
      l'instance NicLinkManager active (reconnect + redémarrage du fen_reader)
      si une partie est en cours, sinon fait une simple vérification comme au
      démarrage ; émet board_ok/board_error en conséquence.
    - alchess.py : suppression du handler reconnect_board devenu mort (l'action
      ne transite plus par menu_queue).
    
    Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/nicsoft/core/board_adapter.py b/nicsoft/core/board_adapter.py
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 166aae3..45ac866 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/nicsoft/core/board_adapter.py
# ── Version APRÈS ce commit.
+++ b/nicsoft/core/board_adapter.py
# ── Zone modifiée : ligne 18 (5 ligne(s)) dans l'ancienne version → ligne 18 (13 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index f1ff748..4208bda 100755
# (avant — fichier suivant)
--- a/nicsoft/niclink/driver.py
# (après — fichier suivant)
+++ b/nicsoft/niclink/driver.py
# ── Zone modifiée : ligne 121 (6 ligne(s)) dans l'ancienne version → ligne 121 (10 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 249 (15 ligne(s)) dans l'ancienne version → ligne 253 (26 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+        is_connected = getattr(self.nl_interface, "is_connected", None)
         while not self._fen_reader_stop.is_set():
             try:
                 _t_lock = time.time()
# ── Zone modifiée : ligne 274 (6 ligne(s)) dans l'ancienne version → ligne 289 (22 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index 2c8fb8c..b7e481a 100644
# (avant — fichier suivant)
--- a/nicsoft/niclink/hid_backend.py
# (après — fichier suivant)
+++ b/nicsoft/niclink/hid_backend.py
# ── Zone modifiée : ligne 34 (6 ligne(s)) dans l'ancienne version → ligne 34 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 72 (7 ligne(s)) dans l'ancienne version → ligne 73 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 81 (6 ligne(s)) dans l'ancienne version → ligne 82 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 101 (7 ligne(s)) dans l'ancienne version → ligne 103 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 109 (6 ligne(s)) dans l'ancienne version → ligne 111 (12 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+    return _connected
 ...
 def get_fen() -> str:
# ── Zone modifiée : ligne 116 (11 ligne(s)) dans l'ancienne version → ligne 124 (20 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index b4ff4e0..b4a6a4a 100644
# (avant — fichier suivant)
--- a/nicsoft/web/alchess.py
# (après — fichier suivant)
+++ b/nicsoft/web/alchess.py
# ── Zone modifiée : ligne 232 (8 ligne(s)) dans l'ancienne version → ligne 232 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index b141be6..7ec3cfb 100644
# (avant — fichier suivant)
--- a/nicsoft/web/server.py
# (après — fichier suivant)
+++ b/nicsoft/web/server.py
# ── Zone modifiée : ligne 253 (6 ligne(s)) dans l'ancienne version → ligne 253 (38 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 272 (6 ligne(s)) dans l'ancienne version → ligne 304 (11 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -272,6 +304,11 @@ def on_action(data):
