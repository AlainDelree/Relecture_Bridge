...
Date:   Thu Aug 13 19:37:48 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    feat: bootstrap_linux.sh — installeur standalone Linux

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/TACHES.md b/TACHES.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 1b0d54a..c98e53d 100644
...
# (index — ignorable)
index 0000000..121222a
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/bootstrap_linux.sh
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (85 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,85 @@
+#!/usr/bin/env bash
+set -e
+
...
# (index — ignorable)
index 262dd30..cc026a6 100755
# (avant — fichier suivant)
--- a/make_release.sh
# (après — fichier suivant)
+++ b/make_release.sh
# ── Zone modifiée : ligne 54 (6 ligne(s)) dans l'ancienne version → ligne 54 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -54,6 +54,7 @@ INCLUDE=(
   LICENSE
   install.sh
   start_alchess.sh
+  bootstrap_linux.sh
   ...
   install_alchess.ps1
# ── Zone modifiée : ligne 164 (7 ligne(s)) dans l'ancienne version → ligne 165 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
