...
Date:   Mon Sep 7 18:07:21 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Ajoute export_chesscoach/ : modules réutilisables extraits pour ChessCoach (issue #252)
    
    Copies adaptées et autonomes de la bibliothèque PGN (library_manager +
    handlers SocketIO), du module d'appel API Claude pour l'analyse (llm_coach,
    avec support d'un fichier de mémoire de coach JSON externe), du wrapper
    Stockfish générique (engine_stockfish, sans Maia/Rodent) et du rendu
    JS/CSS de l'échiquier en mode analyse (board.js/board.css, sans hardware
    Chessnut ni i18n trilingue). NOTES_EXPORT.md détaille origine, dépendances
    et points restant à câbler côté ChessCoach.

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/export_chesscoach/NOTES_EXPORT.md b/export_chesscoach/NOTES_EXPORT.md
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index 0000000..6b3e07b
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- /dev/null
# ── Version APRÈS ce commit.
+++ b/export_chesscoach/NOTES_EXPORT.md
...
diff --git a/export_chesscoach/engine_stockfish.py b/export_chesscoach/engine_stockfish.py
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# (index — ignorable)
index 0000000..0ab95c5
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/export_chesscoach/engine_stockfish.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (485 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,485 @@
+"""
...
# (diff du fichier suivant)
diff --git a/export_chesscoach/library_manager.py b/export_chesscoach/library_manager.py
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# (index — ignorable)
index 0000000..01fcc0e
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/export_chesscoach/library_manager.py
...
@@ -0,0 +1,243 @@
+"""
+library_manager.py — ChessCoach (extrait/adapté d'AlChess)
+Bibliothèque PGN personnelle : collections de parties PGN externes
+(ex. "Carlsen", "Parties pro", "Mes parties lichess"), importées depuis
+lichess/chess.com (fichier multi-parties ou partie unique).
+
+Modèle de données sous DATA_DIR/pgn_library/ :
...
+
+Le dossier DATA_DIR/pgn_library/ est créé automatiquement à la première
+utilisation. Aucun changement fonctionnel par rapport à la version AlChess :
+ce module était déjà mono-utilisateur et sans dépendance pédagogique.
+"""
+
+import datetime
+import io
...
+# ── À câbler côté ChessCoach ─────────────────────────────────────────────────
+# Remplacer cet import par le point d'entrée de config du nouveau projet.
+# Valeur attendue : un pathlib.Path pointant vers le dossier de données de
+# l'application (ex. Path.home() / "ChessCoach" / "data").
...
+
+# ──────────────────────────────────────────────
+# Import PGN (lichess/chess.com, mono ou multi-parties)
+# ──────────────────────────────────────────────
+
+def import_pgn(collection_id: str, content: str) -> dict:
+    """
+    Parse un PGN (une ou plusieurs parties, export lichess/chess.com typique)
+    avec python-chess, append les parties au games.pgn de la collection et
+    met à jour l'index meta.json.
+
...
# (diff du fichier suivant)
diff --git a/export_chesscoach/llm_coach.py b/export_chesscoach/llm_coach.py
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# (index — ignorable)
index 0000000..1776fb9
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/export_chesscoach/llm_coach.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (172 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,172 @@
+"""
...
# (diff du fichier suivant)
diff --git a/export_chesscoach/socketio_pgn_handlers.py b/export_chesscoach/socketio_pgn_handlers.py
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# (index — ignorable)
index 0000000..2b355da
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/export_chesscoach/socketio_pgn_handlers.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (79 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,79 @@
+"""
...
+            emit("pgn_lib_error", {"message": str(e)})
# (diff du fichier suivant)
diff --git a/export_chesscoach/static/board.css b/export_chesscoach/static/board.css
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# (index — ignorable)
index 0000000..44ec60d
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/export_chesscoach/static/board.css
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (145 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,145 @@
+/*
+ * board.css — ChessCoach (extrait/adapté de nicsoft/web/static/css/main.css, AlChess)
+ *
+ * Styles du plateau virtuel, de l'historique des coups, de la barre
+ * d'évaluation Stockfish et des flèches d'annotation, en mode analyse.
+ * Retiré : tout ce qui concernait les écrans pédagogique/exercices/humain
+ * d'AlChess (sélecteurs #ex-board, #retrans-board, .ex-hint, spinner
+ * d'exercice, etc.) — ne restent que les sélecteurs génériques renommés
+ * #board / #arrows-svg / #eval-* pour coller aux IDs utilisés par board.js.
+ */
+
+:root {
+  --bd-size: min(70vh, 560px);
+}
+
+#board {
+  display: grid;
...
+.square.last-move-from       { background: #cdd16f !important; }
+.square.last-move-to         { background: #aaa23a !important; }
+.square.highlight-best-from  { background: #50c8f0 !important; }
+.square.highlight-best-to    { background: #1a9bc0 !important; }
+.square.feedback-bon         { background: #4caf50 !important; }
+.square.feedback-imprecision { background: #ff9800 !important; }
+.square.feedback-erreur      { background: #f44336 !important; }
+.square.feedback-blunder     { background: #9c27b0 !important; }
+
+/* ── Flèches d'annotation (superposées au plateau) ── */
+#arrows-svg {
+  position: absolute;
+  top: 0;
+  left: 0;
...
+#historique { overflow-y: auto; flex: 1; }
+#historique table { width: 100%; border-collapse: collapse; font-size: 0.82rem; }
+#historique td { padding: 2px 4px; white-space: nowrap; color: #1a2a3a; }
+#historique th { padding: 2px 4px; font-weight: 700; color: #1a2a3a; font-size: 0.8rem; border-bottom: 1px solid #a0b8d0; }
+
+.move-chip {
...
+  color: #1a2a3a;
+}
+.move-chip.bon         { color: #1a2a3a; }
+.move-chip.imprecision { color: #cc7700; }
+.move-chip.erreur      { color: #cc2200; }
+.move-chip.blunder     { color: #7b00b0; }
+
+/* ── Barre d'évaluation Stockfish ── */
+.eval-bar-wrap  { display: flex; flex-direction: column; align-items: center; width: 18px; flex-shrink: 0; }
+.eval-score     { font-size: 0.62rem; color: #445; margin-bottom: 2px; white-space: nowrap; }
+.eval-bar-track { flex: 1; width: 14px; border-radius: 4px; overflow: hidden; border: 1px solid #333; background: #111; display: flex; flex-direction: column; }
+.eval-fill-black { background: #222;    width: 100%; flex: 1; transition: flex 0.4s ease; }
+.eval-fill-white { background: #e0e0e0; width: 100%; flex: 1; transition: flex 0.4s ease; }
+
+/* ── Panneau de chat avec le coach ── */
+.coach-drawer {
+  display: none;
...
+.coach-send-btn  { margin: 0; padding: 6px 12px; font-size: 0.82rem; }
+
...
+  width: 16px;
+  height: 16px;
+  border: 3px solid #a0b8d0;
+  border-top-color: #e94560;
+  border-radius: 50%;
+  animation: spin 0.9s linear infinite;
+}
# (diff du fichier suivant)
diff --git a/export_chesscoach/static/board.js b/export_chesscoach/static/board.js
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# (index — ignorable)
index 0000000..7c53b27
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/export_chesscoach/static/board.js
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (622 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,622 @@
+/*
