...
Date:   Sat Aug 22 10:53:01 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Verrouille les accès concurrents à l'état partagé de server.py (issue #213)
    
    server.py tourne en async_mode="threading" avec ~12 threads actifs mais
    n'avait aucun Lock : plusieurs séquences composées (lecture-modification-
    écriture) sur ses globals mutables pouvaient s'entrelacer entre handlers
    déclenchés indépendamment. Introduit 5 verrous ciblés, chacun documenté en
    commentaire avec ce qu'il protège et pourquoi :
    
    - _game_state_lock (RLock) : send_event() (setdefault+append, pop
      conditionnel, lecture de hist[-1]) et on_connect() (lecture cohérente
      fen/history/move/turn/feedback).
    - _board_status_lock : couple (_board_status, _board_error_message),
      toujours écrit/lu ensemble (send_event, on_connect, set_app_state).
    - _session_basket_lock : append+len+pop conditionnel dans on_basket_add(),
      vérification de borne + accès par index dans on_basket_load().
    - _disconnect_timer_lock : cycle vérifier-annuler-(re)créer partagé entre
      on_connect() et on_disconnect().
    - _analyse_llm_lock : incrémentation du compteur de génération
      _analyse_llm_request_id dans on_analyse_llm_ask().
    
    _virtual_board_ref : pas de verrou, TOCTOU corrigé par une lecture unique
    dans une variable locale (pointeur, affectation atomique sous le GIL).
    Caches de disponibilité moteurs (_stockfish/_maia/_rodent) : pas de verrou
    non plus — computation idempotente, un verrou obligerait à le tenir
    pendant l'appel bloquant (handshake UCI/subprocess), ce qui est proscrit.
    _app_state : affectation atomique, aucune séquence composée locale à
    server.py.
    
    Aucun verrou n'est tenu pendant un emit()/appel bloquant — uniquement
    autour de la mutation des structures de données. py_compile OK, tests
    non-e2e/non-hardware au vert (36 passed).
...
index c6a6218..1e6b7a1 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/nicsoft/web/server.py
# ── Version APRÈS ce commit.
+++ b/nicsoft/web/server.py
# ── Zone modifiée : ligne 81 (7 ligne(s)) dans l'ancienne version → ligne 81 (22 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -81,7 +81,22 @@ action_queue: queue.Queue = queue.Queue()
...
# ── Zone modifiée : ligne 92 (12 ligne(s)) dans l'ancienne version → ligne 107 (28 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -92,12 +107,28 @@ menu_queue: queue.Queue = queue.Queue()
...
# ── Zone modifiée : ligne 281 (9 ligne(s)) dans l'ancienne version → ligne 312 (10 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -281,9 +312,10 @@ def _get_game_folders():
 def on_connect():
...
# ── Zone modifiée : ligne 292 (43 ligne(s)) dans l'ancienne version → ligne 324 (67 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -292,43 +324,67 @@ def on_connect():
...
# ── Zone modifiée : ligne 403 (16 ligne(s)) dans l'ancienne version → ligne 459 (18 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -403,16 +459,18 @@ def on_virtual_move(data):
     data = {"uci": "e2e4"}
...
# ── Zone modifiée : ligne 618 (6 ligne(s)) dans l'ancienne version → ligne 676 (14 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -618,6 +676,14 @@ def on_outils_san_to_uci(data):
...
# ── Zone modifiée : ligne 625 (20 ligne(s)) dans l'ancienne version → ligne 691 (21 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -625,20 +691,21 @@ def on_basket_add(data):
...
# ── Zone modifiée : ligne 910 (6 ligne(s)) dans l'ancienne version → ligne 977 (13 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -910,6 +977,13 @@ def on_explorer_tts_stop(_data):
...
# ── Zone modifiée : ligne 937 (8 ligne(s)) dans l'ancienne version → ligne 1011 (9 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -937,8 +1011,9 @@ def on_analyse_llm_ask(data):
...
# ── Zone modifiée : ligne 975 (45 ligne(s)) dans l'ancienne version → ligne 1050 (52 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -975,45 +1050,52 @@ def send_event(event_type: str, data: dict) -> None:
...
# ── Zone modifiée : ligne 1040 (12 ligne(s)) dans l'ancienne version → ligne 1122 (15 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1040,12 +1122,15 @@ def set_app_state(state: str, data: dict = None) -> None:
     if data:
         payload.update(data)
