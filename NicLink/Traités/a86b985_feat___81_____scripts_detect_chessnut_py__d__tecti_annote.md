...
Date:   Tue Jul 28 01:26:03 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    feat: #81 — scripts/detect_chessnut.py, détection auto des modèles Chessnut inconnus
    
    Énumère les appareils HID vendor=2d80, ajoute les idProduct inconnus à
    PRODUCT_IDS dans hid_backend.py et génère la règle udev correspondante sur
    Linux. Sera appelé par l'installeur NSIS (Windows) et au premier démarrage
    (Linux). Bump version 1.0.0 -> 1.3.0 dans nicsoft/pyproject.toml.

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/TACHES.md b/TACHES.md
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index bc008c4..6f6ca71 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/TACHES.md
# ── Version APRÈS ce commit.
+++ b/TACHES.md
# ── Zone modifiée : ligne 49 (6 ligne(s)) dans l'ancienne version → ligne 49 (8 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -49,6 +49,8 @@
...
+version="1.3.0"
 authors=[
     { name = "Alain Delree", email="alain.delree@gmail.com" }
 ]
# (diff du fichier suivant)
...
# (diff du fichier suivant)
diff --git a/scripts/detect_chessnut.py b/scripts/detect_chessnut.py
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# (index — ignorable)
index 0000000..91eaa04
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/scripts/detect_chessnut.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (173 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,173 @@
+#!/usr/bin/env python3
+"""
