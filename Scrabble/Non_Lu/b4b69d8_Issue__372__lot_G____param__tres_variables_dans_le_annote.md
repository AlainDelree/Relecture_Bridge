...
Date:   Wed Aug 5 22:14:37 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #372 (lot G) : paramètres variables dans le script de mesure de force
    
    Vocabulaire par palier (--vocab palier) et forçage indépendant du niveau
    (--vocab-a/-b) ; surcharges stratégiques temporaires --malus/--bonus par camp
    (gestionnaire de contexte, valeurs de production restaurées, jamais modifiées) ;
    comparaison de deux CSV (--comparer) ; docstring à jour avec exemples.
    Chaque camp devient une ConfigJoueur (niveau + vocabulaire + calibrage).
    Script hors pytest ; suite inchangée (874 passed). Déterminisme vérifié.
...
index baf404b..0a17222 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/CHANGELOG.md
# ── Version APRÈS ce commit.
+++ b/CHANGELOG.md
# ── Zone modifiée : ligne 9 (6 ligne(s)) dans l'ancienne version → ligne 9 (42 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+"""Mesure la force relative de deux configurations IA sur des parties complètes (issues #362, #372).
 ...
 Contexte
 --------
# ── Zone modifiée : ligne 8 (7 ligne(s)) dans l'ancienne version → ligne 8 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -8,7 +8,7 @@ coup, sur deux positions figées, à graine variable), mais ne dit rien de la
...
+configurations IA sur ``N`` parties jouées entièrement.
 ...
 --------------------------------------------------
# ── Zone modifiée : ligne 16 (57 ligne(s)) dans l'ancienne version → ligne 16 (125 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -16,57 +16,125 @@ Les deux IA s'affrontent **dans la même partie** (même sac au départ, même
...
# ── Zone modifiée : ligne 82 (16 ligne(s)) dans l'ancienne version → ligne 150 (24 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -82,16 +150,24 @@ import math
 import random
 import sys
 import time
...
# ── Zone modifiée : ligne 111 (6 ligne(s)) dans l'ancienne version → ligne 187 (52 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -111,6 +187,52 @@ MAX_TOURS = 400
...
# ── Zone modifiée : ligne 158 (20 ligne(s)) dans l'ancienne version → ligne 280 (126 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -158,20 +280,126 @@ def _verifier_dictionnaire_installe() -> None:
         )
...
# ── Zone modifiée : ligne 180 (9 ligne(s)) dans l'ancienne version → ligne 408 (10 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -180,9 +408,10 @@ def jouer_une_partie(
...
+        id(joueur_b): (trie_b, config_b, random.Random(graine_ia_b)),
     ...
     tours = 0
# ── Zone modifiée : ligne 193 (13 ligne(s)) dans l'ancienne version → ligne 422 (17 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -193,13 +422,17 @@ def jouer_une_partie(
...
# ── Zone modifiée : ligne 236 (19 ligne(s)) dans l'ancienne version → ligne 469 (32 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -236,19 +469,32 @@ def _formater_duree(secondes: float) -> str:
...
# ── Zone modifiée : ligne 258 (7 ligne(s)) dans l'ancienne version → ligne 504 (9 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -258,7 +504,9 @@ def executer_mesure(
...
+        )
         ...
         jouees = i + 1
# ── Zone modifiée : ligne 273 (23 ligne(s)) dans l'ancienne version → ligne 521 (29 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -273,23 +521,29 @@ def executer_mesure(
             )
...
+                f"score_{label_b}",
                 ...
                 "a_commence",
                 "tours",
# ── Zone modifiée : ligne 297 (19 ligne(s)) dans l'ancienne version → ligne 551 (19 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -297,19 +551,19 @@ def _ecrire_csv(
         )
         for r in resultats:
...
# ── Zone modifiée : ligne 330 (20 ligne(s)) dans l'ancienne version → ligne 584 (14 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -330,20 +584,14 @@ def _afficher_resume(
...
# ── Zone modifiée : ligne 354 (11 ligne(s)) dans l'ancienne version → ligne 602 (101 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -354,11 +602,101 @@ def _afficher_resume(
         print(
...
# ── Zone modifiée : ligne 369 (20 ligne(s)) dans l'ancienne version → ligne 707 (30 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -369,20 +707,30 @@ def _parse_niveau(valeur: str) -> Niveau:
         ) from exc
...
# ── Zone modifiée : ligne 398 (16 ligne(s)) dans l'ancienne version → ligne 746 (70 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -398,16 +746,70 @@ def _construire_parser() -> argparse.ArgumentParser:
