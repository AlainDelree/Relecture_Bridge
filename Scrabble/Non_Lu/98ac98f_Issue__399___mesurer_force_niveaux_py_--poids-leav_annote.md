...
Date:   Sun Aug 9 12:56:08 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #399 : mesurer_force_niveaux.py --poids-leave-a/--poids-leave-b
    
    Étend _strategie_surchargee et ConfigJoueur pour surcharger _POIDS_LEAVE du
    niveau considéré, le temps de la mesure uniquement (même pattern que
    --malus-a/--bonus-a). Permet de mesurer l'effet de la leave value (#395)
    sans éditer ia.py à la main.

# ── Début du diff pour CE fichier précis. a/ = version avant, b/ = version après (identiques si le fichier n'a pas été renommé).
diff --git a/scripts/mesurer_force_niveaux.py b/scripts/mesurer_force_niveaux.py
# ── Identifiants internes git (hash du contenu avant/après). Sans intérêt au quotidien, ignorable.
index a706256..619fdda 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/scripts/mesurer_force_niveaux.py
# ── Version APRÈS ce commit.
+++ b/scripts/mesurer_force_niveaux.py
# ── Zone modifiée : ligne 29 (9 ligne(s)) dans l'ancienne version → ligne 29 (9 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+de production** (voir « Paramètres surchargeables » ci-dessous).
 ...
 ----------------------------------------
# ── Zone modifiée : ligne 44 (12 ligne(s)) dans l'ancienne version → ligne 44 (13 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+  modifié**.
 ...
 ----------------------
# ── Zone modifiée : ligne 127 (6 ligne(s)) dans l'ancienne version → ligne 128 (12 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -127,6 +128,12 @@ Mesure fidèle au jeu réel (chaque niveau sur son palier) ::
...
# ── Zone modifiée : ligne 214 (6 ligne(s)) dans l'ancienne version → ligne 221 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 229 (6 ligne(s)) dans l'ancienne version → ligne 237 (8 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 342 (13 ligne(s)) dans l'ancienne version → ligne 352 (16 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 361 (15 ligne(s)) dans l'ancienne version → ligne 374 (19 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 424 (7 ligne(s)) dans l'ancienne version → ligne 441 (10 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+            config_joueur.poids_leave,
             ...
             coup = ia.choisir_coup(
                 partie.plateau,
# ── Zone modifiée : ligne 778 (6 ligne(s)) dans l'ancienne version → ligne 798 (15 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -778,6 +798,15 @@ def _construire_parser() -> argparse.ArgumentParser:
...
# ── Zone modifiée : ligne 798 (10 ligne(s)) dans l'ancienne version → ligne 827 (18 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
