...
Date:   Tue Aug 4 20:33:55 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #362 : script de mesure de la force relative des niveaux IA
    
    Ajoute scripts/mesurer_force_niveaux.py, outil manuel (hors suite pytest)
    qui fait s'affronter deux niveaux IA sur N parties complètes jouées l'une
    contre l'autre dans la même partie (même sac, même tirage d'ordre) pour
    isoler l'effet de la stratégie du bruit du tirage. Déterministe (graine
    unique par partie dérivant sac/ordre/random.Random par joueur IA), avec
    progression, taux de victoire par niveau (IC 95 %), score moyen, écart
    moyen, matchs nuls et répartition de qui a commencé. Export CSV optionnel.
    
    Validé sur 6 et 10 parties (déterminisme confirmé par diff de deux runs
    identiques). Mentionné dans CONTEXTE.md et CHANGELOG.md.
...
index 1a9e261..d7adc10 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/CHANGELOG.md
# ── Version APRÈS ce commit.
+++ b/CHANGELOG.md
# ── Zone modifiée : ligne 7 (6 ligne(s)) dans l'ancienne version → ligne 7 (25 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index 269efd0..d74e251 100644
# (avant — fichier suivant)
--- a/CONTEXTE.md
# (après — fichier suivant)
+++ b/CONTEXTE.md
# ── Zone modifiée : ligne 38 (8 ligne(s)) dans l'ancienne version → ligne 38 (10 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# (index — ignorable)
index 0000000..7a00086
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/scripts/mesurer_force_niveaux.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (415 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,415 @@
+#!/usr/bin/env python3
