...
Date:   Mon Aug 3 01:33:59 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #345 : rebuild_scrabble.bat — telechargement Actualise et scrabble.zip
    
    Avant ISCC : telecharge actualise.zip (Release v1 AlainDelree/Actualise) et
    extrait Actualise.exe vers C:\Temp\ScrabbleBuild\Actualise.exe (source
    attendue par installeur\scrabble.iss), stoppe et signale en cas d'echec.
    
    Apres ISCC : cree manifest.json ({"build": 1, "supprimer": []}), zippe
    dist\Scrabble\ + manifest.json vers installeur\output\scrabble.zip (nom
    fixe, sans numero de version), verifie sa presence et affiche sa taille.
    scrabble.zip est desormais recopie vers le partage comme Scrabble-Setup.exe.
...
index 0000000..294bede
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- /dev/null
# ── Version APRÈS ce commit.
+++ b/CHANGELOG-345.md
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (17 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,17 @@
+### Ajouté
+
...
# ── Zone modifiée : ligne 64 (7 ligne(s)) dans l'ancienne version → ligne 64 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -64,7 +64,7 @@ echo.
 pushd "%LOCALBUILD%"
...
# ── Zone modifiée : ligne 98 (7 ligne(s)) dans l'ancienne version → ligne 98 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 110 (7 ligne(s)) dans l'ancienne version → ligne 110 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 142 (8 ligne(s)) dans l'ancienne version → ligne 142 (43 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
# ── Zone modifiée : ligne 165 (8 ligne(s)) dans l'ancienne version → ligne 200 (37 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
+echo [9/9] Recopie vers le partage et nettoyage du local...
 ...
 if errorlevel 1 (
# ── Zone modifiée : ligne 177 (12 ligne(s)) dans l'ancienne version → ligne 241 (21 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
