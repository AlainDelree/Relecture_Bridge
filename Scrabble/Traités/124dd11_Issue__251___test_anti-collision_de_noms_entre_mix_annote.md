...
Date:   Fri Jul 24 16:02:55 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #251 : test anti-collision de noms entre mixins ApiJeu
    
    Nouveau fichier tests/test_api_jeu_mixins.py qui vérifie mécaniquement
    qu'aucun nom de méthode n'est défini dans plus d'un mixin ou dans la
    classe ApiJeu de base (doublon = bug silencieux MRO).
    
    Le test exclut intelligemment les stubs de protocole (méthodes avec
    corps `...` utilisées pour l'annotation de type entre mixins).
