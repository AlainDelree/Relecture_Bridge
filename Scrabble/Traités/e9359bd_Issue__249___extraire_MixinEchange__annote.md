...
Date:   Fri Jul 24 15:43:55 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #249 : extraire MixinEchange
    
    Création de src/scrabble/ui/api_echange.py contenant MixinEchange avec
    les 5 méthodes d'échange de lettres :
    - echanger_tout : échange complet du chevalet
    - commencer_echange : entrée en mode échange partiel
    - annuler_echange : sortie du mode échange partiel sans action
    - basculer_echange : marquer/démarquer une lettre pour l'échange
    - echanger_selection : valider l'échange partiel
    
    ApiJeu hérite maintenant de MixinDiffusion, MixinTirageOrdre, MixinPose
    et MixinEchange. En-tête de commentaire précisant les dépendances vers
    MixinDiffusion.
...
index 0000000..c3c617e
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- /dev/null
# ── Version APRÈS ce commit.
+++ b/src/scrabble/ui/api_echange.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (217 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,217 @@
...
# ── Zone modifiée : ligne 1058 (7 ligne(s)) dans l'ancienne version → ligne 1059 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1058,7 +1059,7 @@ def detail_tirage_ordre(
 # --------------------------------------------------------------------------- #
...
# ── Zone modifiée : ligne 1288 (171 ligne(s)) dans l'ancienne version → ligne 1289 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1288,171 +1289,6 @@ class ApiJeu(MixinDiffusion, MixinTirageOrdre, MixinPose):
