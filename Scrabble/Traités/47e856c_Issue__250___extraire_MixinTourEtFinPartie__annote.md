...
Date:   Fri Jul 24 15:57:58 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #250 : extraire MixinTourEtFinPartie
    
    Cinquième et dernier volet du découpage d'ApiJeu.
    
    Créé src/scrabble/ui/api_tour_fin_partie.py avec MixinTourEtFinPartie
    contenant les méthodes extraites :
    - passer, faire_jouer_ia (gestion du tour)
    - _persister_entrees, _finaliser_si_terminee, _journaliser_fin_partie
    - retour_menu, creer_partie_recommencee, preparer_partie_recommencee,
      recommencer (fin de partie)
    
    L'ordre d'héritage place MixinTourEtFinPartie avant MixinPose dans
    la déclaration de classe ApiJeu pour éviter le conflit MRO avec les
    stubs de méthodes définis dans MixinPose.
    
    Tests : 747 passed. py_compile OK. Aucune duplication.
...
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (315 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,315 @@
...
# (index — ignorable)
index 95d1180..a7bc7b2 100644
# (avant — fichier suivant)
--- a/src/scrabble/ui/jeu.py
# (après — fichier suivant)
+++ b/src/scrabble/ui/jeu.py
# ── Zone modifiée : ligne 82 (6 ligne(s)) dans l'ancienne version → ligne 82 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -82,6 +82,7 @@ from scrabble.ui.api_diffusion import MixinDiffusion
...
# ── Zone modifiée : ligne 1059 (7 ligne(s)) dans l'ancienne version → ligne 1060 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1059,7 +1060,7 @@ def detail_tirage_ordre(
 # --------------------------------------------------------------------------- #
...
# ── Zone modifiée : ligne 1289 (278 ligne(s)) dans l'ancienne version → ligne 1290 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1289,278 +1290,6 @@ class ApiJeu(MixinDiffusion, MixinTirageOrdre, MixinPose, MixinEchange):
