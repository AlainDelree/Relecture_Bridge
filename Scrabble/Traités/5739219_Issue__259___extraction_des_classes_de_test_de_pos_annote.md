...
Date:   Fri Jul 24 17:04:52 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #259 : extraction des classes de test de pose vers test_jeu_pose.py
    
    Déplace les 8 classes de test demandées + TestApiJeuDiffusionConfidentialite
    (9 classes au total) vers tests/test_jeu_pose.py.
    
    Centralise les helpers partagés (_DicoMots, _placement, _FenetreEspionne)
    dans tests/_aides_test_jeu.py pour éviter la duplication.
...
index 79df4dd..1bf3885 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/tests/_aides_test_jeu.py
# ── Version APRÈS ce commit.
+++ b/tests/_aides_test_jeu.py
# ── Zone modifiée : ligne 16 (6 ligne(s)) dans l'ancienne version → ligne 16 (35 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -16,6 +16,35 @@ class _DicoFactice:
         return True
...
# (index — ignorable)
index 7d64fd5..7715a20 100644
# (avant — fichier suivant)
--- a/tests/test_jeu.py
# (après — fichier suivant)
+++ b/tests/test_jeu.py
# ── Zone modifiée : ligne 16 (15 ligne(s)) dans l'ancienne version → ligne 16 (19 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
...
-from scrabble.regles.lettres import JOKER
 ...
 from scrabble.ui.jeu import (
     ApiJeu,
# ── Zone modifiée : ligne 176 (95 ligne(s)) dans l'ancienne version → ligne 180 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -176,95 +180,6 @@ class TestApiJeuChargementDiffere:
...
# ── Zone modifiée : ligne 1085 (489 ligne(s)) dans l'ancienne version → ligne 1000 (8 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1085,489 +1000,8 @@ class TestApiJeuHelpersCoquilleUnifiee:
         assert supprimees == []
...
# ── Zone modifiée : ligne 1973 (164 ligne(s)) dans l'ancienne version → ligne 1407 (3 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1973,164 +1407,3 @@ class TestZoneTravailEcran:
...
# (index — ignorable)
index 0000000..16640eb
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/tests/test_jeu_pose.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (725 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,725 @@
