...
Date:   Fri Jul 24 16:45:03 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #256 : extraction des classes de test de coup vers test_jeu_coup.py
    
    Déplace TestConstruireCoup, TestJouerPlacements, TestSimulerCoup et
    TestSymetrieSensLettreUnique depuis test_jeu.py vers un nouveau fichier
    test_jeu_coup.py. Les helpers _DicoMots et _placement sont dupliqués dans
    les deux fichiers (car utilisés indépendamment). Les imports inutilisés
    sont retirés de test_jeu.py.
...
index 976b34b..0c063bb 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/tests/test_jeu.py
# ── Version APRÈS ce commit.
+++ b/tests/test_jeu.py
# ── Zone modifiée : ligne 18 (7 ligne(s)) dans l'ancienne version → ligne 18 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -18,7 +18,7 @@ from scrabble.dictionnaire.dictionnaire import Trie
...
+from scrabble.moteur.plateau_partie import Tuile
 from scrabble.persistance import (
     STATUT_TERMINEE,
     demarrer_suivi,
# ── Zone modifiée : ligne 29 (15 ligne(s)) dans l'ancienne version → ligne 29 (12 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -29,15 +29,12 @@ from scrabble.regles.lettres import JOKER
...
# ── Zone modifiée : ligne 265 (11 ligne(s)) dans l'ancienne version → ligne 262 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -265,11 +262,6 @@ class TestApiJeuChargementDiffere:
...
# ── Zone modifiée : ligne 285 (323 ligne(s)) dans l'ancienne version → ligne 277 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -285,323 +277,6 @@ def _placement(ligne: int, colonne: int, lettre: str, joker: bool = False) -> di
...
diff --git a/tests/test_jeu_coup.py b/tests/test_jeu_coup.py
# ── Ce fichier n'existait pas avant ce commit : il vient d'être créé.
new file mode 100644
# (index — ignorable)
index 0000000..f1405dc
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/tests/test_jeu_coup.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (348 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,348 @@
