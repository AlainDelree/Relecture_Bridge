...
Date:   Fri Jul 24 17:25:10 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #264 : extraction des classes de test d'intégration vers test_jeu_integration.py
    
    - Création de tests/test_jeu_integration.py avec les 5 classes de test de
      bout en bout : TestApiPersisteLesActions, TestApiRepriseBoutEnBout,
      TestApiFinaliseEnFinDePartie, TestPersistanceEchecResteVisible,
      TestApiJeuHelpersCoquilleUnifiee
    - Ajout d'un commentaire d'en-tête expliquant que ces tests couvrent
      volontairement plusieurs mixins à la fois (scénarios transversaux)
    - Nettoyage de test_jeu.py : ne conserve que la docstring avec un index
      des 10 fichiers issus du découpage
    - Vérification : 0 classe dans test_jeu.py, 54 classes réparties dans les
      10 fichiers de découpage, 749 tests passent
...
index 644c0d3..f892053 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/tests/test_jeu.py
# ── Version APRÈS ce commit.
+++ b/tests/test_jeu.py
# ── Zone modifiée : ligne 5 (366 ligne(s)) dans l'ancienne version → ligne 5 (17 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -5,366 +5,17 @@ Couvre :
...
diff --git a/tests/test_jeu_integration.py b/tests/test_jeu_integration.py
...
+++ b/tests/test_jeu_integration.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (357 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,357 @@
