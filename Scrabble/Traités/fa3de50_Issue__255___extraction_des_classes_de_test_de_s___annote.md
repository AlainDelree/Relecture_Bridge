...
Date:   Fri Jul 24 16:40:01 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #255 : extraction des classes de test de sérialisation
    
    Déplace 15 classes de tests de sérialisation d'état vers
    tests/test_jeu_serialisation.py :
    - TestSerialiserCase, TestSerialiserPlateau, TestSerialiserJoueurPublic
    - TestSerialiserChevalet, TestEtatPublic, TestCompterHumains
    - TestIndexHumainReference, TestCalculerPositions, TestCalculerAvatars
    - TestIndexPanneauInteractif, TestEtatPublicExpositionTour
    - TestSerialiserDetailScore, TestSerialiserHistorique
    - TestClasserScoreTotal, TestEvaluerScoreTotal
    
    Les helpers _joueurs_humains, _partie_quatre_joueurs, _echanger_une_lettre
    et _poser_chat_au_centre sont également déplacés vers le nouveau fichier.
    
    Les imports de test_jeu.py sont allégés en conséquence.
...
index 2e38b2a..976b34b 100644
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- a/tests/test_jeu.py
# ── Version APRÈS ce commit.
+++ b/tests/test_jeu.py
# ── Zone modifiée : ligne 1 (12 ligne(s)) dans l'ancienne version → ligne 1 (12 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1,12 +1,12 @@
...
+``test_jeu_serialisation.py`` (issue #255).
 ...
 import json
# ── Zone modifiée : ligne 19 (7 ligne(s)) dans l'ancienne version → ligne 19 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -19,7 +19,6 @@ from scrabble.moteur.ia import Niveau
...
-from scrabble.moteur.score import DetailMot, DetailScore
 from scrabble.persistance import (
     STATUT_TERMINEE,
     demarrer_suivi,
# ── Zone modifiée : ligne 27 (205 ligne(s)) dans l'ancienne version → ligne 26 (22 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -27,205 +26,22 @@ from scrabble.persistance import (
     reprendre_partie,
 )
...
# ── Zone modifiée : ligne 866 (354 ligne(s)) dans l'ancienne version → ligne 682 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -866,354 +682,6 @@ class TestApiPoserMot:
 # --------------------------------------------------------------------------- #
...
# ── Zone modifiée : ligne 1879 (86 ligne(s)) dans l'ancienne version → ligne 1347 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1879,86 +1347,6 @@ class TestPasserTour:
 # --------------------------------------------------------------------------- #
...
# ── Zone modifiée : ligne 2027 (222 ligne(s)) dans l'ancienne version → ligne 1415 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -2027,222 +1415,6 @@ class TestJouerToursIaUi:
...
# ── Zone modifiée : ligne 3555 (105 ligne(s)) dans l'ancienne version → ligne 2727 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -3555,105 +2727,6 @@ class TestZoneTravailEcran:
...
# (index — ignorable)
index 0000000..b94c09a
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/tests/test_jeu_serialisation.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (955 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,955 @@
