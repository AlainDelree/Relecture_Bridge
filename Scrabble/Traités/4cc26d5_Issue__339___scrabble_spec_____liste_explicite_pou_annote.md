...
Date:   Fri Jul 31 17:42:56 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #339 : scrabble.spec — liste explicite pour data/dictionnaire + garde-fou de taille
    
    collect_tree(data/dictionnaire) embarquait sans filtre tout ce que
    contenait le dossier au moment du build (os.walk récursif), alors que
    ce dossier est gitignoré : un dump de travail de 8,2 Go déposé par
    erreur (wiktionnaire-kaikki/, jamais lu à l'exécution) a ainsi produit
    des installeurs Windows de ~400 Mo en 25+ minutes le 31/07/2026, et
    deux clones du même commit pouvaient produire deux installeurs
    différents selon l'état local de la machine de build.
    
    - ELEMENTS_DICTIONNAIRE / ELEMENTS_DICTIONNAIRE_TIERS énumèrent
      désormais explicitement les entrées de premier niveau embarquées
      (~87 Mo mesuré, contre ~88 Mo attendu) ; les sous-dossiers restent
      parcourus récursivement par collect_tree.
    - verifier_taille_datas() fait échouer le build au-delà de 200 Mo,
      avec le détail des plus gros contributeurs.
    - Élément optionnel absent (caches régénérables, personnalisations) :
      avertissement. Aucun dictionnaire tiers (ODS8 ni Hunspell) trouvé :
      échec du build.
    - CHANGELOG.md créé (n'existait pas) avec l'entrée correspondante.
    
    Vérifié par exécution isolée du préambule du .spec (sans PyInstaller) :
    45 fichiers, 87,1 Mo, et déclenchement confirmé des trois chemins
    (avertissement optionnel absent, échec seuil dépassé, échec aucun
    dictionnaire tiers). py_compile scrabble.spec : OK.
...
# (index — ignorable)
index 37df93c..4f115cc 100644
# (avant — fichier suivant)
--- a/scrabble.spec
# (après — fichier suivant)
+++ b/scrabble.spec
# ── Zone modifiée : ligne 25 (16 ligne(s)) dans l'ancienne version → ligne 25 (29 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -25,16 +25,29 @@ gelés, donc placer le dossier ``web`` à la destination ``scrabble/ui/web``
...
+contributeurs.
 ...
 import os
# ── Zone modifiée : ligne 63 (14 ligne(s)) dans l'ancienne version → ligne 76 (144 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -63,14 +76,144 @@ def collect_tree(src_relatif, dest_relatif):
     return entrees
