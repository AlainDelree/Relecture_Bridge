...
Date:   Fri Jul 24 17:13:56 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #261 : extraction des classes de test de tour/fin de partie vers test_jeu_tour_fin_partie.py
...
@@ -19,7 +19,6 @@ from scrabble.moteur.partie import Joueur, Partie, creer_partie
...
# ── Zone modifiée : ligne 30 (13 ligne(s)) dans l'ancienne version → ligne 29 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -30,13 +29,7 @@ from scrabble.persistance import (
     reprendre_partie,
 )
...
# ── Zone modifiée : ligne 178 (131 ligne(s)) dans l'ancienne version → ligne 171 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -178,131 +171,6 @@ class TestApiJeuChargementDiffere:
...
# ── Zone modifiée : ligne 339 (312 ligne(s)) dans l'ancienne version → ligne 207 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -339,312 +207,6 @@ class TestFermetureMutuellePopovers:
...
# ── Zone modifiée : ligne 732 (21 ligne(s)) dans l'ancienne version → ligne 294 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -732,21 +294,6 @@ class TestApiJeuHelpersCoquilleUnifiee:
...
# (index — ignorable)
index 0000000..10b45f1
# (avant — fichier suivant)
--- /dev/null
# (après — fichier suivant)
+++ b/tests/test_jeu_tour_fin_partie.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (461 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,461 @@
