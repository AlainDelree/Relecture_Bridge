...
Date:   Fri Jul 24 15:37:03 2026 +0200

# ── Message de commit : résumé de l'intention du changement, écrit par celui qui a committé.
    Issue #248 : extraire MixinPose
    
    Crée src/scrabble/ui/api_pose.py avec class MixinPose contenant les
    méthodes de pose de lettres déplacées depuis ApiJeu : selectionner_lettre,
    poser_lettre_en_attente, _ajouter_placement, retirer_lettre_en_attente,
    remplacer_ou_retirer_lettre_en_attente, annuler_pose, poser_mot,
    _normaliser_placement, verifier_coup, verifier_mot.
    
    ApiJeu hérite maintenant de MixinDiffusion, MixinTirageOrdre et MixinPose.
    En-tête du mixin documentant les dépendances vers MixinDiffusion
    (_diffuser, _refuser_hors_tour) et vers les méthodes d'ApiJeu
    (_persister_entrees, _journaliser_fin_partie, _finaliser_si_terminee).
    
    Les imports de journal et charger_config utilisent scrabble.ui.jeu comme
    module de référence pour maintenir la compatibilité avec les tests qui
    patchent ces références.
...
index 0000000..1386e74
# ── Version AVANT ce commit (/dev/null = le fichier n'existait pas).
--- /dev/null
# ── Version APRÈS ce commit.
+++ b/src/scrabble/ui/api_pose.py
# ── Zone modifiée : ligne 0 (0 ligne(s)) dans l'ancienne version → ligne 1 (466 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -0,0 +1,466 @@
...
# ── Zone modifiée : ligne 1057 (7 ligne(s)) dans l'ancienne version → ligne 1058 (7 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1057,7 +1058,7 @@ def detail_tirage_ordre(
 # --------------------------------------------------------------------------- #
...
# ── Zone modifiée : ligne 1287 (415 ligne(s)) dans l'ancienne version → ligne 1288 (6 ligne(s)) dans la nouvelle. Une ligne '+' = ajoutée, '-' = supprimée, sans signe = contexte inchangé.
@@ -1287,415 +1288,6 @@ class ApiJeu(MixinDiffusion, MixinTirageOrdre):
